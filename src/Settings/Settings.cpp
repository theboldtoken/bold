...
const char DEFAULT_WALLET_FILE_NAME[] = "bold.wallet";
...
Settings::Settings() : m_p2pBindPort(0), m_cmdLineParser(nullptr) {
  m_defaultPoolList << "pool.boldprivate.network:4442";
...
void Settings::init() {
  QFile cfgFile(getDataDir().absoluteFilePath("boldwallet.cfg"));
...
bool Settings::isStartOnLoginEnabled() const {
  ...
  QString autorunFilePath = autorunDir.absoluteFilePath("boldwallet.plist");
  ...
  QString autorunFilePath = autorunDir.absoluteFilePath("boldwallet.desktop");
  ...
  res = autorunSettings.contains("BoldWallet") &&
    !QDir::fromNativeSeparators(autorunSettings.value("BoldWallet").toString().split(' ')[0]).compare(QCoreApplication::applicationFilePath());
#endif
  return res;
}
...
void Settings::setStartOnLoginEnabled(bool _enable) {
  {
    ...
    autorunSettings.setValue("Label", "org.bold.boldwallet");
    ...
    if (_enable) {
      ...
      autorunFile.write("Name=Bold Wallet\n");
    ...
    if (_enable) {
      ...
      autorunSettings.setValue("BoldWallet", appPath);
    } else {
      autorunSettings.remove("BoldWallet");
    }
#endif
  }
...
void Settings::setUrlHandler() {
  QWriteLocker lock(&m_lock);
  QSettings protocolSettings("HKEY_CURRENT_USER\\Software\\Classes\\bold", QSettings::NativeFormat);
  protocolSettings.setValue(".", "URL:bold");
  protocolSettings.setValue("URL Protocol", "");
  QSettings iconSettings("HKEY_CURRENT_USER\\Software\\Classes\\bold\\DefaultIcon", QSettings::NativeFormat);
  iconSettings.setValue(".", QDir::toNativeSeparators(QCoreApplication::applicationFilePath()));
  QSettings openSettings("HKEY_CURRENT_USER\\Software\\Classes\\bold\\shell\\open\\command", QSettings::NativeFormat);
...
void Settings::saveSettings() const {
  QFile cfgFile(QDir(m_cmdLineParser->getDataDir()).absoluteFilePath("boldwallet.cfg"));
